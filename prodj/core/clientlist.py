...
class ClientList:
    def __init__(self, prodj):
        ...
        self.prodj = prodj
        #midi_interface = MidiClock("CH345")
        self.midi_interface = MidiClock()
        #self.midi_interface.open("CH345", 0)
        self.midi_interface.open(preferred_name="Midi Through")
        #self.midi_interface.setBpm(175)
        self.midi_interface.start()
        
        #self.midi_interface.open()
    ...
    def eatStatus(self, status_packet):
        ...
        if c.beat != new_beat and new_beat != 0:
            c.beat = new_beat
            client_changed = True
            #TODO: maybe jumpin for sending note
            if 'master' in c.state:
                try:
                    self.midi_interface.send_note(59)
                    self.midi_interface.send_note(59+new_beat) #189,190,191,192
                    print('-------------')
                    print('beat',new_beat)
                    print('-------------')
